    ...
    for (int i = 0; i < s_store.count; i++) {
        ...
        for (int j = 0; j < HTTPS_MAX_SESSIONS; j++) {
            if (!s_sessions[j].valid ||
                ...
                strncpy(s_sessions[j].user, user, HTTPS_USER_LEN - 1);
                s_sessions[j].user[HTTPS_USER_LEN - 1] = '\0';
...
int https_auth_verify_pass_for_session(const char *token_hex, const char *pass) {
    ...
    char found_user[HTTPS_USER_LEN] = {0};
    uint32_t now = xTaskGetTickCount();
    for (int i = 0; i < HTTPS_MAX_SESSIONS; i++) {
        if (!s_sessions[i].valid) {
            continue;
        ...
        if (memcmp(s_sessions[i].token, token, HTTPS_SESSION_TOKEN_LEN) == 0) {
            strncpy(found_user, s_sessions[i].user, HTTPS_USER_LEN - 1);
            break;
        }
    }
    if (found_user[0] == '\0') {
    ...
    for (int i = 0; i < s_store.count; i++) {
        ...
        if (strncmp(s_store.accounts[i].user, found_user, HTTPS_USER_LEN) != 0) {
            continue;
        }
        return (memcmp(s_store.accounts[i].pass_hash, hash, HTTPS_HASH_LEN) == 0) ? 1 : 0;
    }
    return 0;
}
